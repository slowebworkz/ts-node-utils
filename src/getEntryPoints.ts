...
const packages = process.env['PACKAGES']?.split(',').map(pkg => pkg.trim()).filter(Boolean) ?? [];

const filterExisting = (paths: string[]) => paths.filter(existsSync);

// Warn if any listed packages are missing entry points
const missing = packages.filter(pkg => !existsSync(`packages/${pkg}/src/index.ts`));
if (missing.length) {
  console.warn(`Warning: missing entry points for packages: ${missing.join(', ')}`);
}
